<script>
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.status == 200) {
                        ...
                        $('#parent_name').html(res.student.parent.title + ' ' + res.student
                            .parent.first_name + ' ' + res.student.parent.last_name);
        ...
        $(document).ready(function() {
         ...
         //fetch not offering subjects item
         $(document).on('click', '.sno', function(e) {
            e.preventDefault();

            let student_id = $(this).data('student_id');
            let student_name = $(this).data('student_name');

            $('#sno_modal_title').html('Subjects not offered By '+student_name);
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route('get-subjects_not_offering') }}',
                data: {
                    'student_id': student_id,
                },
                success: function(res) {
                    ...
                    html = $('#sno_id').html(html);

                    $('#sno_loading_div').addClass('d-none');
                    $('#sno_content_div').removeClass('d-none');
                }
            });
         ...
         //submit subject not offering
         $(document).on('submit', '#sno_form', function(e) {
            e.preventDefault();

            let formData = new FormData($('#sno_form')[0]);

            spinner =
                '<div class="spinner-border" style="height: 20px; width: 20px;" role="status"><span class="sr-only">Loading...</span></div> Submitting . . .'
            $('#sno_btn').html(spinner);
            $('#sno_btn').attr("disabled", true);
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('save-subjects_not_offering') }}",
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {

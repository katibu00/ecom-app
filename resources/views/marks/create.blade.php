                    ...
                    <div class="card-body">
                                    ...
                                    <div>
                                        <div class="d-flex justify-content-between align-items-center fw-bolder mb-0">
                                            ...
                                            <span style="font-size: 12px; font-weight: normal;"
                                                id="remaining">{{ $students->count() - @$marked }} remaining</span>
                                        </div>
                                        <div class="progress mb-50" style="height: 8px">
                                            <div class="progress-bar progress-animated" role="progressbar"
                                                style="width: {{ (@$marked / @$students->count()) * 100 }}%"
                                                aria-valuenow="6" aria-valuemax="100" aria-valuemin="0"></div>
                                        </div>
                        ...
                        <form class="form" action="{{ route('marks.create.fetch') }}" method="POST">
                            ...
                            <div class="row">
                                    ...
                                    <select class="form-select form-select-sm mb-3" id="assign_id" name="assign_id">
                                        ...
                                        @foreach ($subjects as $subject)
                                            <option value="{{ $subject->id }}"
                                                ...
                                                {{ $subject->designation == 1 ? 'Compulsory' : 'Optional' }}</option>
                                        @endforeach
                                    </select>
                                    @error('assign_id')
                                ...
                                <div class="col-sm-4 mb-1">
                                    <select id="marks_category" class="form-select form-select-sm mb-3"
                                        name="marks_category">
                                        ...
                                        @if (isset($students))
                                            ...
                                            @if(is_numeric($marks_category))
                                                ...
                                                @endphp
                                                <option value="{{ $ca->code }}" selected>{{ $ca->code }}</option>
                                            @endif
                            ...
                            <form id="main_form">
                                <div class="marks_table ">
                                    <div class="table-responsive mt-2">
                                        <table class="table table-responsive-md">
                                            ...
                                            <tbody>
                                                @foreach ($students as $key => $student)
                                                    <tr>
                                                        <td>{{ $key + 1 }}</td>
                                                        <td>{{ @$student->login ? $student->login : @$student->student->login }}
                                                        </td>
                                                        ...
                                                        <td><input type="hidden" name="user_id[]"
                                                                ...
                                                                type="{{ @$student->absent == 'abs' ? 'text' : 'number' }}"
                                                                max="60" class="form-control input-rounded"
                                                                id="marks" name="marks"
                                                                data-user_id="{{ @$student->student_id }}"
                                        ...
                                        <input type="hidden" id="subject_id_send" name="subject_id"
                                            value="{{ @$subject_id }}">
                                        <input type="hidden" id="class_id_send" name="class_id"
                                            value="{{ @$class_id }}">
                                    ...
                                    <div class="col-12 text-center mt-1">
                                        ...
                                        <button type="submit"
                                            class="btn btn-outline-danger initialize_btn d-none">Initialize</button>
                    ...
                    success: function(response) {
                        ...
                        if (response.ca_schemes) {
                            ...
                            response.ca_schemes.forEach(function(caScheme) {
                                marksCategorySelect.append('<option value="' + caScheme
                                    .id +
                                    '" {{ @$marks_category == ' + caScheme.id + ' ? 'selected' : '' }}>' +
                                    caScheme.code + ' - ' + caScheme.desc +
                                    '</option>');
                            });
                            marksCategorySelect.append(

<script type="text/javascript">
    $(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $.each(res.invoices, function(key, invoice) {
                        ...
                        html += '<option value="' + invoice.id + '">' + '#' +
                            invoice.number + ' - ' + invoice.student.first_name +
                            ' ' + middle + ' ' + invoice.student.last_name +
                            '</option>';
                    });
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $.each(res.recommededs, function(key, recommeded) {
                        html += '<div class="form-check custom-checkbox mb-0">' +
                            '<input type="checkbox" class="form-check-input optional_fee" name="additional_fee" value="' +
                            recommeded.id + '" data-amount="' + recommeded.amount +
                            '">' +
                            '<label class="form-check-label" for="same-address">' +
                    ...
                    $.each(res.optionals, function(key, optional) {
                        html += '<div class="form-check custom-checkbox mb-0">' +
                            '<input type="checkbox" class="form-check-input optional_fee" name="additional_fee" value="' +
                            optional.id + '" data-amount="' + optional.amount +
                            '" >' +
                            '<label class="form-check-label" for="same-address">' +
                            optional.fee_category.name + ' - Optional - ' + optional
                        ...
                        if (res.additionals.length >= 1) {
                            $.each(res.additionals, function(key, additional) {
                                html +=
                                    ...
                                    '<input type="checkbox" class="form-check-input optional_fee" checked disabled name="additional_fee" value="' +
                                    additional.id + '" data-amount="' + additional
                                    .amount + '" >' +
                                    '<label class="form-check-label" for="same-address">' +
                                    additional.fee_category.name +
                ...
                .toLocaleString());
                    ...
                    if (res.initial == 'no') {
                        ...
                        $('.discounted_amount').html('&#8358;' + res.discounted_amount
                            .toLocaleString());
        ...
        $("#add_payment_amount").keyup(function() {
            ...
            $('.modal_balance').html('&#8358;' + (parseInt(balance) - parseInt(payment_amount))
                .toLocaleString());

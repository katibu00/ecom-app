<!DOCTYPE html>
@php
    $students = App\Models\User::select('id', 'first_name', 'parent_id', 'middle_name', 'last_name', 'class_id')
        ->with('parent', 'class')
        ->where('class_id', $class_id)
        ->where('status', 1)
        ->get();
    $accounts = App\Models\BankAccount::select('bank', 'number', 'name')
    ...
    @foreach ($students as $key => $student)
        @php
            ...
            $fees = App\Models\FeeStructure::with('fee_category')
                ->select('student_type', 'fee_category_id', 'amount', 'priority')
                ...
                ->get();
            $total_invoice = App\Models\FeeStructure::with('fee_category')
                ->select('student_type', 'fee_category_id', 'amount')
                ->where('school_id', $school->id)
                ->where('class_id', $class_id)
                ->where('student_type', $invoice->student_type)
                ->sum('amount');
        ...
        <div class="invoice-print p-5">
            ...
            <div class="row d-flex justify-content-between mb-4">
                <div class="col-sm-6 w-50">
                    <h6>Invoice To:</h6>
                    <p class="mb-1"><strong>Student:
                        </strong>{{ @$student->first_name . ' ' . @$student->middle_name . ' ' . @$student->last_name }}</p>
                    <p class="mb-1"><strong>Parent:
                        </strong>{{ @$student->parent->title . ' ' . @$student->parent->first_name . ' ' . @$student->parent->last_name }}
                    </p>
                    <p class="mb-1"><strong>Class: </strong>{{ @$student->class->name }}</p>
                    <p class="mb-1"><strong>Session: </strong>{{ @$school->session->name }}</p>

                    <p class="mb-0"><strong>Term: </strong>{{ $termText }}</p>
                    ...
                    <table>
                        <tbody>
                            <tr>
                                ...
                                <td><strong>&#8358;{{ number_format($total_invoice + $invoice->pre_balance - $invoice->discount, 2) }}</strong>
                                </td>
                            </tr>
                            <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="2" class="align-top px-4 py-3">
                                ...
                                <span>Thanks for Choosing {{ $school->name }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                ...
                                @if ($invoice->discount != 0)
                                    <p class="fw-bold mb-2">&#8358;{{ number_format($invoice->discount, 2) }}</p>
                                @endif
                                {{-- <p class="fw-bold mb-2">$50.00</p> --}}
                                <p class="fw-bold mb-2">
                                    &#8358;{{ number_format($total_amount + $invoice->pre_balance - $invoice->discount, 2) }}
                                </p>
                            </td>
                        </tr>
                    </tbody>
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <span>The amount of Mandatory Fees is {{ number_format($total_mandatory, 0) }}. You can pay by
                        making bank transfer to any of the above bank account or in-person in the school
                        premises.</span>
                </div>
            </div>
        </div>
